            ...
            <section className="glass-card rounded-xl p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-2 min-w-0">
                  ...
                  <h2 className="font-display text-2xl md:text-3xl font-bold">{company.name}</h2>
                  {company.description && (
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <div className="font-display text-4xl font-bold text-primary tabular-nums">{company.scout_score}</div>
                  <div className="flex items-center gap-3">
                ...
                {history.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={history} margin={{ top: 8, right: 12, left: 0, bottom: 8 }}>
                      <CartesianGrid stroke="hsl(220 12% 16%)" strokeDasharray="3 3" />
                      <XAxis dataKey="date" tick={{ fill: 'hsl(220 8% 55%)', fontSize: 10 }} />
                      <YAxis domain={[0, 100]} tick={{ fill: 'hsl(220 8% 55%)', fontSize: 10 }} />
                      <Tooltip
                        contentStyle={{ background: 'hsl(220 15% 8%)', border: '1px solid hsl(220 12% 16%)', borderRadius: 8, fontSize: 12 }}
                        labelStyle={{ color: 'hsl(220 8% 75%)' }}
                      />
                      <Line type="monotone" dataKey="score" stroke="hsl(38 100% 50%)" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <section className="glass-card rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-border/40">
                  ...
                  <h3 className="font-display text-sm font-bold uppercase tracking-wider">Catalysts</h3>
                </div>
                  ...
                  {catalysts.length === 0 ? (
                    ...
                    <div key={c.id} className="p-4">
                      <div className="flex items-baseline justify-between gap-2">
                        ...
                        <span className="text-[10px] font-mono uppercase text-muted-foreground shrink-0">{fmtDate(c.event_date)}</span>
                      </div>
